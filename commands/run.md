...
| 파일 | 동작 |
|------|------|
| `docs/requirements.md` | 읽어서 Phase 1의 seed로 사용. 기존 요구사항(기능 포함) 요약 출력 후 "변경된 사항만 말씀해 주세요"로 질문 간소화 |
...
`Installed Agents & Skills` 테이블과 `Agent Triggers` 테이블의 `<스택별 ... 추가>` 부분은
`docs/requirements.md`의 `Agents Needed`를 확인하여 아래 매핑 테이블에 따라 동적으로 채운다:
...
**스택별 에이전트 감지 hook** (`docs/requirements.md`의 `Agents Needed`에 해당 에이전트가 있는 경우, 위 PostToolUse 배열에 추가):
...
```json
{
  ...
  "hooks": [
    {
      "type": "command",
      "command": "bash -c 'printf \"%s\" \"$CLAUDE_TOOL_INPUT\" | grep -qiE \"\\\"file_path\\\".*/(routes|controllers|handlers|api|endpoints|views)/\" && echo \"🏗️  서버/API 코드 감지 — backend-developer 에이전트를 호출하세요\" || true'"
    }
  ]
}
```
...
```json
{
  ...
  "hooks": [
    {
      "type": "command",
      "command": "bash -c 'printf \"%s\" \"$CLAUDE_TOOL_INPUT\" | grep -qiE \"\\\"file_path\\\".*(components|pages|views).*\\.(tsx|jsx|vue|svelte)\" && echo \"🎨 UI 컴포넌트 코드 감지 — frontend-developer 에이전트를 호출하세요\" || true'"
    }
  ]
}
```
...
```json
{
  ...
  "hooks": [
    {
      "type": "command",
      "command": "bash -c 'printf \"%s\" \"$CLAUDE_TOOL_INPUT\" | grep -qiE \"\\\"file_path\\\".*(train|model|pipeline|dataset|evaluate|inference)\" && echo \"🤖 ML 코드 감지 — ml-engineer 에이전트를 호출하세요\" || true'"
    }
  ]
}
```
...
### 6. 추가 권장 플러그인 (Setup Complete 내 출력)

`docs/requirements.md`의 스택/도메인 정보를 분석하여 아래 매핑 기준으로 관련 플러그인을 추천합니다.
**설치는 사용자가 직접 실행**합니다 (자동 설치 불가).

모든 플러그인은 `/plugin marketplace add <owner>/<repo>` 형식으로 설치합니다.
...
**출력 규칙**:
- 해당 조건에 맞는 플러그인만 골라 Setup Complete 메시지 안에 출력
- 최대 5개까지만 노출 (우선순위: LSP > UI/UX > 테스트 > 보안 > DevOps > 기타)
- 이미 번들에서 커버되는 영역은 제외 (예: `backend-patterns` 번들 스킬이 있으면 별도 추천 불필요)
...
| 플러그인 | 효과 | 설치 명령 |
|----------|------|-----------|
| typescript-lsp | 타입 오류 인라인 제공 | /plugin marketplace add anthropics/typescript-lsp |
| ... | ... | ... |
```
